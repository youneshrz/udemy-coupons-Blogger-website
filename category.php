...
include_once 'include/function.php';
    $category= str_replace('-',' ',str_replace('_',' & ',$_GET['cat']));
                   // pagination ========
...
$start = ($pageno-1) * $no_of_records_per_page;
                // get id of category by name of category
                $sql=$con->prepare("SELECT * FROM categories WHERE Cat_name='$category' ");
                $sql->execute();
                $categories=$sql->fetch();
                // get all recored related by this category
                $stmt=$con->prepare("SELECT * FROM posts WHERE Approve=0 AND Catid={$categories['Cat_id']} order by Post_id desc limit  $start,$no_of_records_per_page  ");
                $stmt->execute();
                $rows=$stmt->fetchAll();
               //pagination=======
               $nomber_total_recored=$con->prepare("SELECT * FROM posts WHERE Approve=0 AND Catid={$categories['Cat_id']} order by Post_id desc ");
...
$nomber_total_pages=ceil($nomber_total_recored/$no_of_records_per_page);
    ...
    $site->execute();
    $site=$site->fetch();
    $title='Classfreecourses-'. $category;
    $description=  $site['Site_description'];
    $image=$base_url."/logo1.png";
    $_url=$base_url.'/category/'.$category;
    $canonical= $_url;
    $index='index';
    $follow='follow';
...
<div class="small-nav">
<a href="index.php">Home</a> <i class="fa fa-chevron-right color-chevron"></i> category <i class="fa fa-chevron-right color-chevron"></i> <?php echo $category; ?>
</div>
<br>
<?php                        

    echo  "<span style='color: #6c757d'>".$category." ( ".$nomber_total_recored . " )  resultats. </span>" ;
    ?>
...
<div style="display:none" class="h5-filter" id="all"></div>
<div class="row">
<?php 
...
<?php }
                    ...
                    echo "<div style='width:100%;' class=' alert alert-info text-center'> There are no resultats Matching with ''".$category."'' </div>";
                }
...
<a href="<?php echo $base_url; ?>/category/<?php echo $category ?>/1"><span class="first-page"> << </span></a>
<a href="<?php echo $base_url; ?>/category/<?php echo $category ?>/<?php echo $pageno -1 ; ?>"><span class="perv-page">prev</span></a>
</div>
<?php } ?>
<?php if($pageno == $nomber_total_pages){
                        }else{ ?>
<div class="pagination-left">
<a href="<?php echo $base_url; ?>/category/<?php echo $category ?>/<?php echo $pageno+1 ; ?>"><span class="next-page">next</span></a>
<a href="<?php echo $base_url; ?>/category/<?php echo $category ?>/<?php echo $nomber_total_pages ?>"><span class="last-page"> >> </span></a>
</div>
<?php }?>
</div>
</div>
